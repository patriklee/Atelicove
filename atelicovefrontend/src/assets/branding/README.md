...
The splash assets expose `atelicove-mark`, `petal-center`, `petal-upper-left`, `petal-upper-right`, `petal-lower-left`, and `petal-lower-right`. Every petal group uses its fill-box edge nearest the shared lower-center growth point as its transform origin. Timing and animation belong in the consuming CSS or React component.
