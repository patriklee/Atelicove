  ...
  return (
    <Paper component="form" onSubmit={onSubmit} sx={{ p: 3, mb: 4 }}>
      ...
      <Stack spacing={2}>
        ...
        <FormControl><InputLabel>Teams</InputLabel><AppSelect multiple value={form.teamIDs.map(Number)} label="Teams" onChange={event => onChange({ teamIDs: event.target.value.map(Number) })}>{teams.map(team => <MenuItem key={team.teamID} value={team.teamID}>{team.teamName || `Team #${team.teamID}`}</MenuItem>)}</AppSelect></FormControl>
