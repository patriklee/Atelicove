...
const alertDialogContent = {
  'overdue-deadlines': {
    title: 'Overdue Project Deadlines',
    subtitle: 'Project deadlines that have passed and still require attention.',
    ariaLabel: 'Overdue project deadlines',
    icon: icons.deadline,
  },
  'work-order-review': {
    title: 'Work Orders Awaiting Review',
    subtitle: 'Work orders currently waiting for administrative review.',
    ariaLabel: 'Work orders awaiting review',
    icon: icons.workOrders,
  },
  'project-review': {
    title: 'Projects Awaiting Review',
    subtitle: 'Projects currently waiting for administrative review.',
    ariaLabel: 'Projects awaiting review',
    icon: icons.projects,
  },
  'projects-without-work': {
    title: 'Projects Without Assigned Work',
    subtitle: 'Projects that do not currently have associated work orders.',
    ariaLabel: 'Projects without assigned work',
    icon: icons.projects,
  },
  'unassigned-work': {
    title: 'Unassigned Work Orders',
    subtitle: 'Active work orders that do not currently have assigned workers.',
    ariaLabel: 'Unassigned work orders',
    icon: icons.workOrders,
  },
};

const workOrderContext = order => (
  order.projectName
  || order.project?.projectName
  || order.company?.companyName
  || 'No project or company'
);

function WorkOrderAlertRows({ records, showAssignment, onNavigate }) {
  return records.map(order => (
    <TableRow key={order.workOrderID} hover>
      ...
      {showAssignment && <TableCell>Unassigned</TableCell>}
    </TableRow>
  ));
}

function AlertDetailDialog({ alert, open, onClose, onNavigate }) {
  const content = alertDialogContent[alert?.id];
  const records = alert?.records || [];

  const renderTable = () => {
    if (alert.id === 'overdue-deadlines') {
      return (
        <Table size="small" aria-label={content.ariaLabel} stickyHeader>
          <TableHead>
            <TableRow>
              <TableCell>Project</TableCell>
              <TableCell>Action item / deadline</TableCell>
              <TableCell>Due date</TableCell>
              <TableCell>Status</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {records.map(deadline => (
              <TableRow key={`${deadline.projectID}-${deadline.actionItemID ?? deadline.itemText}`} hover>
                <TableCell>
                  <Button
                    variant="text"
                    size="small"
                    onClick={() => onNavigate(`/admin/projects/${deadline.projectID}`)}
                    sx={{ p: 0, minWidth: 0, textTransform: 'none', fontWeight: 600 }}
                  >
                    {deadline.projectName}
                  </Button>
                </TableCell>
                <TableCell>{deadline.itemText || 'Untitled action item'}</TableCell>
                <TableCell sx={{ whiteSpace: 'nowrap' }}>{formatDueDate(deadline.dueDate)}</TableCell>
                <TableCell><Chip size="small" color="error" variant="outlined" label="Overdue" /></TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
    ...
    if (alert.id === 'work-order-review' || alert.id === 'unassigned-work') {
      const showAssignment = alert.id === 'unassigned-work';
      return (
        <Table size="small" aria-label={content.ariaLabel} stickyHeader>
          <TableHead>
            <TableRow>
              ...
              {!showAssignment && <TableCell>Assigned Workers</TableCell>}
              <TableCell>Status</TableCell>
              {showAssignment && <TableCell>Assignment</TableCell>}
            </TableRow>
          </TableHead>
          <TableBody>
            <WorkOrderAlertRows
    ...
    const showBudget = alert.id === 'project-review';
    return (
      <Table size="small" aria-label={content.ariaLabel} stickyHeader>
        <TableHead>
          <TableRow>
            ...
            {showBudget && <TableCell>Budget Health</TableCell>}
            <TableCell>Status</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {records.map(project => (
            <TableRow key={project.projectID} hover>
              ...
              {showBudget && <TableCell><BudgetHealth health={getBudgetHealth(project)} /></TableCell>}
  ...
  return (
    ...
    >
      <DialogTitle id="alert-detail-dialog-title" sx={{ pb: 1 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start" spacing={2}>
          <Stack direction="row" spacing={1.25} alignItems="flex-start">
            {content?.icon && (
              <Box sx={{ color: 'brand.secondary', display: 'flex', mt: 0.25 }}>
                <AppIcon icon={content.icon} />
              </Box>
            )}
            <Box>
              <Typography variant="h5" component="span">{content?.title || 'Alert Details'}</Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
                {content?.subtitle}
              </Typography>
            </Box>
          </Stack>
          <Chip size="small" variant="outlined" label={`${records.length} ${records.length === 1 ? 'result' : 'results'}`} />
        </Stack>
      </DialogTitle>
      <DialogContent dividers sx={{ p: 0, overflow: 'auto' }}>
        {records.length ? (
          <TableContainer sx={{ maxHeight: '65vh' }}>{renderTable()}</TableContainer>
        ) : (
          <Box sx={{ py: 7, px: 3, textAlign: 'center' }}>
            <Typography variant="h6">Nothing needs attention here.</Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.75 }}>
              The dashboard data has changed and there are no matching records now.
            </Typography>
          </Box>
        )}
      </DialogContent>
...
export default function AdminDashboard() {
  ...
  const selectedAlert = useMemo(
    () => (selectedAlertID ? getWorkQueueItem(data, selectedAlertID) : null),
      ...
      >
        ...
        <Stack direction="row" spacing={1} alignItems="center" sx={{ width: { xs: '100%', md: 'auto' } }}>
          <DashboardSearch
            ...
            onNavigate={navigate}
          />
          <DashboardAlerts items={workQueue} onSelect={item => setSelectedAlertID(item.id)} />
        </Stack>
      </Stack>
