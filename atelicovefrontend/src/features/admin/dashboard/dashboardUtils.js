import { getWorkOrderActualPrice, getWorkOrderWorkers, normalizeWorker } from '../../../model';
...
function getWorkQueueEntries(data = {}, now = new Date()) {
  ...
  const reviewProjects = projects.filter(project => project.projectStatus === 'IN_REVIEW');
  const projectsWithoutWork = projects.filter(project => !(project.workOrders || []).length);
  const unassignedWorkOrders = workOrders.filter(order => (
    ['OPEN', 'IN_PROCESS'].includes(order.status) && !getWorkOrderWorkers(order).length
  ));
  ...
  return [
    ...
    },
    {
      ...
      count: reviewWorkOrders.length,
      records: reviewWorkOrders,
      severity: 'warning',
    ...
    },
    {
      ...
      count: projectsWithoutWork.length,
      records: projectsWithoutWork,
      severity: 'warning',
      path: '/admin/projects/active',
    },
    {
      ...
      count: unassignedWorkOrders.length,
      records: unassignedWorkOrders,
      severity: 'error',
      path: '/admin/manage-workorders',
    },
  ];
...
export function getWorkQueueItem(data = {}, alertID, now = new Date()) {
  return getWorkQueueEntries(data, now).find(entry => entry.id === alertID) || null;
}

export function getWorkQueue(data = {}, now = new Date()) {
  return getWorkQueueEntries(data, now).filter(entry => entry.count > 0);
}
