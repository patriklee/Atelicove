  ...
  test('renders overdue deadlines and urgent queue entries from real data', () => {
    ...
    expect(getWorkQueue(data, now)).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        count: 1,
        severity: 'error',
        records: [expect.objectContaining({ actionItemID: 3, projectID: 1 })],
      }),
      expect.objectContaining({
        id: 'unassigned-work',
        count: 1,
        severity: 'error',
        records: [expect.objectContaining({ workOrderID: 9 })],
      }),
    ]));

    getWorkQueue(data, now).forEach(entry => expect(entry.count).toBe(entry.records.length));
  });
